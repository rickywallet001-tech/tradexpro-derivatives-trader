...
describe('<CancelDeal />', () => {
    ...
    it('should render the component with checkbox and popover', async () => {
    ...
    it('should call onToggleCancellation if user clicked on Deal cancellation checkbox and onToggleDealCancel returned true', async () => {
        render(mockCancelDeal());

        const checkbox_with_cancellation = screen.getByText(deal_cancellation);
        await userEvent.click(checkbox_with_cancellation);

        expect(mockOnToggleCancellation).toBeCalled();
    });
    it('should show <DealCancellationWarningDialog /> if (has_take_profit || has_stop_loss) && should_show_cancellation_warning is equal to true and user clicked on Deal cancellation checkbox', async () => {
    ...
    it('should call toggleCancellationWarning function if cancellation warning checkbox inside of <DealCancellationWarningDialog /> was clicked', async () => {
        const new_mocked_props = { ...default_mocked_props, has_take_profit: true };
        ...
        await userEvent.click(checkbox_with_show);

        expect(new_mock_store.ui.toggleCancellationWarning).toBeCalled();
    });
    it('<DealCancellationWarningDialog /> should not be visible if Cancel button was clicked', async () => {
    ...
    it('<DealCancellationWarningDialog /> should not be visible if Confirm button was clicked', async () => {
        const new_mocked_props = { ...default_mocked_props, has_stop_loss: true };
        const new_mock_store = { ...default_mock_store, ui: { should_show_cancellation_warning: true } };
        const mock_root_store = mockStore(new_mock_store);
        render(mockCancelDeal(mock_root_store, new_mocked_props));
